...
class TextMelDataset(torch.utils.data.Dataset):
    def __init__(
            self, filelist_path, add_blank=True, n_fft=1024, n_mels=80, sample_rate=22050,
        ...
        self.filepaths_and_text = [
            [i[0], self.get_text(i[1])]
            for i in self.filepaths_and_text
        ]
    ...
    def get_mel(self, filepath):
        ...
        mel = mel_spectrogram(
            audio, self.n_fft, self.n_mels, self.sample_rate, self.hop_length, self.win_length,
            self.f_min, self.f_max, center=False
        ).squeeze()
        return mel
