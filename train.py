...
from model.tts import Diffusion
...
def load_checkpoints(
        model, path2checkpoint
) -> None:
    checkpoint = torch.load(path2checkpoint)
    model_dict = model.state_dict()
    print(
        sum([model_dict and model_dict[k].shape == v.shape for k, v in checkpoint.items()]) /
        len(checkpoint)
    )
...
if __name__ == "__main__":
    ...
    test_dataset = TextMelDataset(
        valid_filelist_path, add_blank, n_fft, n_feats, sample_rate, hop_length, win_length, f_min, f_max
    )

    checkpoint = r"checkpts/grad_100.pt"

    print('Initializing model...')
    model = GradTTS(
        nsymbols, 1, None, n_enc_channels, filter_channels, filter_channels_dp, n_heads, n_enc_layers,
        enc_kernel, enc_dropout, window_size, n_feats, dec_dim, beta_min, beta_max, pe_scale
    )

    load_checkpoints(model, checkpoint)

    # model.decoder = Diffusion(n_feats, dec_dim, 1, None, beta_min, beta_max, pe_scale)
