...
warnings.filterwarnings("ignore")  # simple way


class Normalizer:
    def __init__(
            self, device=None, jit_model='norm.pt'
    ):
        self.src2tgt = None
        self.tgt_vocab = None
        self.src_vocab = None
        ...
        self.model = torch.load(jit_model, map_location=self.device)
        self.model.eval()
        self.max_len = 150
    ...
    def __call__(self, text):
        ...
        #
        # 1. Full text is separated by "ending" symbols (\n\t?!.) to sentences;
        # 2. Long sentences additionally separated to chunks: by spaces or just dividing too long words
