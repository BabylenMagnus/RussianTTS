...
from g2p.rules_for_graphem import RulesForGraphemes


class Grapheme2Phonem(RulesForGraphemes):
    def __init__(self, users_mode='Modern', exception_for_nonaccented=False):
        ...
        self.__function_words_1 = {
            'без', 'близ', 'в', 'из', 'меж', 'над', 'об', 'под', 'пред', 'перед', 'чрез', 'через', 'из-под', 'по-над'
        }

        self.__function_words_2 = {
            'бы', 'б', 'де', 'ли', 'же', '-то', '-ка', '-либо', '-нибудь', '-таки'
        }
        ...
        with codecs.open(file_name, mode='r', encoding='utf-8', errors='ignore') as dictionary_file:
            ...
            while len(cur_line):
                ...
                if len(prepared_line):
                    ...
                    assert any(
                        [c in (self.mode.all_russian_letters | {'-', '+'}) for c in word_original]), error_message
                    assert any(
                        [c in (self.mode.all_russian_letters | {'-', '+'}) for c in word_transformed]), error_message
    ...
    def __call__(self, text):
        return self.phrase_to_phonemes(text)
